...
- 이모티콘 누끼는 바깥 흰 배경만 제거한다. 문구/글자 획 주변의 밝은 종이 질감은 보존해 글자 내부가 투명하게 뚫리지 않게 한다.
- 캐릭터 얼굴, 몸, 말풍선 내부처럼 의도된 흰색 면은 외곽선이 일부 열려 있어도 투명 처리하지 않고 흰색으로 보존한다.
...
- 이모티콘 이미지 변경은 PNG 해상도, RGBA 투명도, 파일 크기를 `public/students/00` 기준과 비교하고, 문구가 깨지거나 글자 속/흰색 얼굴 면이 과하게 투명 처리되지 않았는지 직접 확인한다.
